<script>
  ...
  let back_img = back;
  let front_img = "";
  // Pas de préfixe si : URL http(s), base64 data:, ou chemin absolu /
  let img_base = (img.startsWith("http") || img.startsWith("data:") || img.startsWith("/") || img.startsWith("blob:"))
    ? ""
  ...
  const popover = () => {
    const rect = thisCard.getBoundingClientRect(); // get element's size/position
    let delay = 100;
    // Utiliser le panneau parent comme référence pour éviter le débordement
    const panel = thisCard.closest(".panel") || thisCard.closest("main") || document.body;
    const panelRect = panel.getBoundingClientRect();
    let scaleW = (panelRect.width  / rect.width)  * 0.82;
    let scaleH = (panelRect.height / rect.height) * 0.82;
    let scaleF = 1.5; // réduit de 1.75 → 1.5 pour rester dans le panneau
    setCenter();
    if (firstPop) {
      delay = 1000;
